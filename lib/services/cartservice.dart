...
class CartService {
  ...
  static Future<void> addProductInstance({
    ...
    final variantsPrice = parsedVariants.fold(0.0, (sum, v) => sum + (v['prezzo'] as double));
    cartItems.add({
      'cod': productCode,
      'des': productName,
      'qta': 1,
   ...
   static Future<void> updateProductInstance({
    ...
    if (existingItemIndex >= 0) {
      ...
      cartItems.add({
        ...
        'variants_prz': variantsPrice,  // Use the calculated price sum
        'timestamp': DateTime.now().millisecondsSinceEpoch,
