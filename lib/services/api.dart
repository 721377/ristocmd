...
class ApiService {
  ...
  static Future<List<Map<String, dynamic>>> fetchSalas() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print(
            '[ApiService] Failed to fetch Salas. Status code: ${response.statusCode}');
  ...
  static Future<List<Map<String, dynamic>>> fetchTavolos(int salaId) async {
    final url =
        Settings.buildApiUrl('${Settings.getTavoloEndpoint}/$salaId/pv/$pv');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print(
            '[ApiService] Failed to fetch Tavolos. Status code: ${response.statusCode}');
  ...
  static Future<List<Map<String, dynamic>>> fetchMovtable(int tavoloId) async {
    final url = Settings.buildApiUrl(
        '${Settings.getMovtemEndpoint}/$pv/tavolo/$tavoloId');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print(
            '[ApiService] Failed to fetch Tavolo ORDER. Status code: ${response.statusCode}');
  ...
  static Future<Map<String, dynamic>> tableByid(int tavoloId) async {
    final url =
        Settings.buildApiUrl('${Settings.gettablebyidEndpoint}/$tavoloId');
...
//categori api
  static Future<List<Map<String, dynamic>>> fetchGruppi() async {
    final url =
        Settings.buildApiUrl('${Settings.getGruppiEndpoint}/$pv/da_palmare/1');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print(
            '[ApiService] Failed to fetch Gruppi. Status code: ${response.statusCode}');
  ...
  static Future<List<Map<String, dynamic>>> fetchArtByGruppi(
      id_gruppi, cod_lis) async {
    final url = Settings.buildApiUrl(
        '${Settings.getArtByGruppo}/cod_lis/$cod_lis/id_cat/$id_gruppi/des/0');
    print(
        '[ApiService] Fetching articoli of the groupe : $id_gruppi from: $url');
    try {
      final response = await http.get(Uri.parse(url));
      if (response.statusCode == 200) {
        print('[ApiService] Successfully fetched articoli.');
        List<Map<String, dynamic>> articoliBygruppi =
            List<Map<String, dynamic>>.from(json.decode(response.body));
        ...
        print(
            '[ApiService] Failed to fetcharticoliGruppo. Status code: ${response.statusCode}');
  ...
  static Future<List<Map<String, dynamic>>> fetchVarianti(id_gruppi) async {
    final url =
        Settings.buildApiUrl('${Settings.getvariantiByGruppo}/$id_gruppi/des');
    print(
        '[ApiService] Fetching varianti of the groupe : $id_gruppi from: $url');
    try {
      final response = await http.get(Uri.parse(url));
      if (response.statusCode == 200) {
        print('[ApiService] Successfully fetched varianti.');
        List<Map<String, dynamic>> variantiBygruppi =
            List<Map<String, dynamic>>.from(json.decode(response.body));
        ...
        print(
            '[ApiService] Failed to fetchvariantiGruppo. Status code: ${response.statusCode}');
...
static Future<List<Map<String, dynamic>>> fetchImpostazionipalm() async {
  final url = Settings.buildApiUrl(Settings.getImpostazioniPalm);
  print('[ApiService] Fetching ImpostazioniPalm at URL: $url');

  try {
    final response = await http.get(Uri.parse(url));
    if (response.statusCode == 200) {
      print('[ApiService] Successfully fetched ImpostazioniPalm.');
      ...
      print('[ApiService] Failed to fetch ImpostazioniPalm. Status code: ${response.statusCode}');
      throw Exception('Failed to load ImpostazioniPalm');
    }
  } catch (e) {
    print('[ApiService] Error fetching ImpostazioniPalm: $e');
