...
class _TableDetailsPageState extends State<TableDetailsPage> {
  ...
  String _timeFilter = 'all'; // 'all', 'today', 'last_hour'
  ...
  Future<void> _loadLocalOrders() async {
    ...
    setState(() {
      _localOrders = localOrders.map((e) {
        ...
        map['timer_start'] = map['timer_start'] ?? DateTime.now().toIso8601String();
...
Future<void> _saveCustomerCount(int count) async {
  ...
  if (widget.onUpdateTableStatus != null) {
    widget.onUpdateTableStatus!(widget.table['id'].toString(), count > 0 ? 'occupied' : 'free');
  }
  ...
  void _checkAutoOpenModal() {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final hasOnlineOrders =
          widget.orders.any((o) => o['mov_descr'] != 'COPERTO');
...
void _showCustomerCountModal() {
 ...
 Widget buildFilterSelector() {
  ...
  return Container(
    margin: EdgeInsets.symmetric(horizontal: 24, vertical: 12),  // Reduced vertical margin
                    ...
                    children: filters.map((filter) {
                      ...
                      return Padding(
                        ...
                        child: GestureDetector(
                          ...
                          child: Container(
                            width: itemWidth,
                            height: 36,  // Reduced height of each filter option
                            alignment: Alignment.center,
                            decoration: BoxDecoration(
                              border: Border.all(
                                color: isSelected
                            ...
                            child: Text(
                              ...
                              style: TextStyle(
                                  fontWeight: FontWeight.w600,
                                  fontSize: 12,  // Reduced font size for each label
                                  color: isSelected
...
Widget build(BuildContext context) {
  ...
  final existingOnlineCards = _getExistingOrderCards(onlineOrderGroups, _localOrders);
  final newOfflineCards = _getNewOrderCards(offlineOrderGroups, _localOfflineOrders);
  final existingOfflineCards = _getExistingOrderCards(offlineOrderGroups, _localOfflineOrders);
...
Widget _buildOrderItemRow(Map<String, dynamic> order, Color color, DateTime orderTime) {
  ...
  final hasVariant = variantDescription != null && variantDescription.isNotEmpty;
